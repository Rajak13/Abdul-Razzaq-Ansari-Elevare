...
export class AdminAuditService {
  ...
  async createAuditLog(
    ...
    try {
      ...
      // Create audit log entry - hash will be generated by database trigger
      const result = await client.query(
        `INSERT INTO audit_logs (admin_id, action_type, target_entity, target_id, details, ip_address, user_agent, timestamp)
         VALUES ($1, $2, $3, $4, $5, $6, $7, CURRENT_TIMESTAMP)
         RETURNING id, hash`,
        [adminId, actionType, targetEntity, targetId, JSON.stringify(details), ipAddress, userAgent]
      );

      const auditId = result.rows[0].id;
      const auditHash = result.rows[0].hash;
