...
async function runMigrations() {
  try {
    ...
    // Get migrations directory - check both locations
    let migrationsDir = path.join(__dirname, '../../migrations');
    
    // If running from dist, migrations might be in dist/db/migrations
    ...
    if (!fs.existsSync(migrationsDir)) {
      ...
      console.error('  - ' + path.join(__dirname, '../db/migrations'));
