...
ALTER TABLE notes ADD COLUMN IF NOT EXISTS summary_model VARCHAR(50);
ALTER TABLE notes ADD COLUMN IF NOT EXISTS content_hash VARCHAR(64);

-- Create index for efficient summary queries
CREATE INDEX IF NOT EXISTS idx_notes_summary_generated ON notes(summary_generated_at) 
